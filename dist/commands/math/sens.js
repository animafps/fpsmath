...
module.exports = class sens extends discord_js_commando_1.Command {
    constructor(client) {
        super(client, {
            name: "sens",
            ...
            group: "math",
            memberName: "sens",
            description: "Converts cm/360(default), deg/mm or inch/360 to a game sensitivity",
            details: "Converts cm/360(default), deg/mm or inch/360 to a game sensitivity \nTo see the Supported games use the `games` Command",
            examples: [
        ...
        switch (args.flags) {
            case "-deg": {
                const output = ((args.cpi *
                    parseFloat(array_1.getObject(args.yaw.toLowerCase(), "yaw")) *
                    60) /
                    args.cm).toFixed(args.dp);
                msg.reply(output);
                break;
            }
            case "-MPI": {
                const output = ((24.5 * args.cm) /
                    (args.cpi * parseFloat(array_1.getObject(args.yaw.toLowerCase(), "yaw")))).toFixed(args.dp);
                msg.reply(output);
                break;
            }
            case "-inch": {
